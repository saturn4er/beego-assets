...
func (this *Asset) parse() error {
  ...
  for {
    ...
    if strings.HasPrefix(line, prefix) {
      ...
      this.Original_files = append(this.Original_files, file)
    }
  }//foreach line
  return nil
}
  ...
  if v, ok := parsedAssets[assetName]; ok {
    if asset, ok := v[assetType]; ok {
      if Config.ProductionMode {
        //in production mode, we dont detect changes
        return asset, nil
      }
      
      if !beego_cache.GetCache().FileChanged(asset.assetPath) {
        Debug("Asset from cache %s", asset.assetPath)
        //check if each asset file changed
        changed := false
        for _, file := range asset.Original_files {
          if (beego_cache.GetCache().FileChanged(file)) {
            Warning("included changed %s", file)
            changed = true
            break
          }
        }
